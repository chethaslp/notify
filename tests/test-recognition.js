...
async function testRecognitionLogic(element, skipPdfCheck = true) {
  ...
  try {
    ...
    if (/2020\s*scheme/.test(normalizedString) || /2024\s*scheme/.test(normalizedString)) {
      result.passed = true;
    ...
    try {
      ...
      if (((/2020\s*scheme/.test(pdfLower) || /2024\s*scheme/.test(pdfLower)) && /b\.?tech/.test(pdfLower))) {
        result.passed = true;
